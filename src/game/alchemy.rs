...
fn init_alchemy_table(
	...
	commands.spawn((
		...
		Restitution::new(0.2),
		NamedMaterials(smallvec![NamedMaterial::new("Table", Color::rgb(0.4,0.2,0.04))]),
		Collider::compound(vec![(Vec3::Y*0.53,Quat::IDENTITY,Collider::cuboid(1.04, 0.05, 0.5))]),
	...
	commands.spawn((
		...
		SceneBundle {
			...
			transform: Transform::from_xyz(3.0, 5.0, 0.0)
	...
	commands.spawn((
		...
		LockedAxes::TRANSLATION_LOCKED_Z,
		Collider::compound(vec![(Vec3::Y * 0.05,Quat::IDENTITY,Collider::capsule(Vec3::ZERO, Vec3::Y * 0.3, 0.05))]),
		SecondWorldBundle::default()
	));
	...
	commands.spawn((
		...
		SceneBundle {
			...
			transform: Transform::from_xyz(7.0, 5.0, 0.0)
				.with_scale(Vec3::splat(2.5)),
			..default()
		},
		Damping {
...
fn rotate_head(
	...
	let desired_rotation = Quat::slerp(Quat::IDENTITY,Quat::from_mat3(&Mat3::from_cols(right, up, forward)), 0.5);

	head_transform.rotation = Quat::slerp(head_transform.rotation, desired_rotation, (1.0 - 0.001f64.powf(time.delta_seconds_f64())) as f32)
}
...
fn consume_eaten(
	...
	mut eaten_query: Query<(Entity, &mut ItemSize, &mut Transform, &Item), (With<Eaten>, Without<PlayerHead>)>,
	time: Res<Time>,
	game_assets: Res<GameAssets>,
	sound: Res<Audio>,
...
fn consume_cauldroned(
	...
	for (entity, mut item_size, mut transform, mut item, named_mats) in &mut cauldroned_query {
		...
		if item_size.current_size() < 0.08 {
			...
			transform.translation = Vec3::lerp(transform.translation, cauldron_pos, (1.0 - 0.0001f64.powf(time.delta_seconds_f64())) as f32)
		}
	}
}
...
fn check_eaten(
	mut commands: Commands,
	head_query: Query<&Transform,With<PlayerHead>>,
	mut item_query: Query<(Entity, &GlobalTransform, &mut ItemSize, &CenterOfMass), (With<Item>, Without<Eaten>, Without<Cauldroned>, Without<DroppedItem>)>,
	...
	for (item_entity, global_transform, mut item_size, local_com) in &mut item_query {
		...
		let length = (head_transform.translation.xy().extend(0.0) + HEAD_OFFSET - global_com.xy().extend(0.0)).length_squared();
...
fn check_mortar_crushing(
	...
	for (mortar_transfrom, mut mortar) in &mut mortar_query {
		...
		for (pestle_transfrom, pestle_velocity, local_com) in &pestle_query {
			...
			let world_com = pestle_transfrom.compute_matrix().mul_vec4(local_com.0.extend(1.0));
...
fn mash_ingredient(
	mut commands: Commands,
	mut ingridient_query: Query<(Entity, &GlobalTransform, &CenterOfMass, &mut Ingredient), (Without<DroppedItem>, With<Item>)>,
	...
	for (mortar_transform, mortar_local_com, _) in mortar_query.iter().filter(|m| m.2.0) {
		for (entity, item_transform, item_local_com, mut ingredient) in &mut ingridient_query {
			let item_world_com = item_transform.compute_matrix().mul_vec4(item_local_com.0.extend(1.0)).xyz();
			let mortar_world_com = mortar_transform.compute_matrix().mul_vec4(mortar_local_com.0.extend(1.0)).xyz();
					...
					*amount += time.delta_seconds();
					...
					if *amount > 0.35 {
						commands.entity(entity)
							...
							.insert(Collider::compound(vec![(Vec3::new(0.0,0.15,0.0), Quat::IDENTITY, Collider::round_cone(0.132,0.225,0.05))]));

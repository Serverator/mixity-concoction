use bevy::{render::{view::RenderLayers, camera::ScalingMode}, core_pipeline::{clear_color::ClearColorConfig, fxaa::Fxaa}, gltf::Gltf};
...
fn spawn(
	...
	let mut camera_transform = Transform::from_rotation(Quat::from_rotation_x(-0.2) * Quat::from_rotation_y(-0.25));
	...
	commands.spawn((
		...
		Collider::compound(vec![
			(Vec3::new(0.0,0.02,0.0), Quat::IDENTITY, Collider::cuboid(0.27,0.012,0.5)),
			(Vec3::new(-0.27,0.18,0.0), Quat::IDENTITY, Collider::cuboid(0.012,0.18,0.5)),
			(Vec3::new(0.27,0.18,0.0), Quat::IDENTITY, Collider::cuboid(0.012,0.18,0.5)),
			(Vec3::new(-0.252,0.53,0.0), Quat::from_rotation_z(-6.0f32.to_radians()), Collider::cuboid(0.012,0.173,0.5)),
			(Vec3::new(0.252,0.53,0.0), Quat::from_rotation_z(6.0f32.to_radians()), Collider::cuboid(0.012,0.173,0.5)),
			(Vec3::new(-0.32,0.17,0.0), Quat::IDENTITY, Collider::cuboid(0.04,0.13,0.5)),
			(Vec3::new(0.32,0.17,0.0), Quat::IDENTITY, Collider::cuboid(0.04,0.13,0.5)),
		]),
	));

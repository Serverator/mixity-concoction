...
use super::{items::{DroppedItem, Item}, ingredient::{Ingredient, Grind}, world::SpawnableInstance, player::Player};
...
pub fn spawn_arrows(
	...
	commands.spawn((
		...
		NamedMaterials(smallvec![
			NamedMaterial::new("Arrow", Color::WHITE)
		])
	));
	...
	commands.spawn((
		...
		NamedMaterials(smallvec![
			NamedMaterial::new("Arrow", Color::rgb(0.7,0.7,1.0))
		])
	));
}
...
pub fn rotate_arrow(
	...
	mut arrow: Query<(&mut Transform, &mut Visibility), (With<Arrow>, Without<RareArrow>,Without<Player>)>,
	mut rare_arrow: Query<(&mut Transform, &mut Visibility), (With<RareArrow>, Without<Arrow>,Without<Player>)>,
	ingredient_query: Query<(&Transform, &Ingredient, &SpawnableInstance), (Without<DroppedItem>,Without<Player>,Without<Arrow>,Without<RareArrow>)>,
	...
	*arrow_visibility = if effect.is_some() { Visibility::Visible } else { Visibility::Hidden };
	*rare_arrow_visibility = if rare_effect.is_some() { Visibility::Visible } else { Visibility::Hidden };
	...
	if rare_effect.is_some() {
		let closest_rare = ingredient_query
			...
			.map(|a| (a.0, a.0.translation.distance_squared(player_transform.translation)))
			.min_by(|a,b| a.1.total_cmp(&b.1));

		if let Some((closest,distance)) = closest_rare {
			rare_arrow_transform.rotation = Quat::slerp(rare_arrow_transform.rotation, Quat::from_rotation_arc(Vec3::Z, (player_transform.translation - closest.translation).xz().extend(0.0).xzy().normalize()), 1.0 - 0.001f32.powf(time.delta_seconds()));
			rare_arrow_transform.translation = player_transform.translation + rare_arrow_transform.forward() * 1.0f32.min(distance.sqrt() - 1.5);
		} else {
	...
	if effect.is_some() {
		...
		if let Some((closest,distance)) = closest {
			arrow_transform.rotation = Quat::slerp(arrow_transform.rotation, Quat::from_rotation_arc(Vec3::Z, (player_transform.translation - closest.translation).xz().extend(0.0).xzy().normalize()), 1.0 - 0.001f32.powf(time.delta_seconds()));
			arrow_transform.translation = player_transform.translation + arrow_transform.forward() * 1.0f32.min(distance.sqrt() - 1.5);
		} else {
		...
		if time.elapsed_seconds() > *last_boom {
			...
			*last_boom = time.elapsed_seconds() + rng.gen_range(0.5..3.0);
			for mut velocity in &mut velocities {
				velocity.linvel += Vec3::new(rng.gen_range(-1.0..1.0),rng.gen_range(-1.0..1.0),rng.gen_range(-1.0..1.0)) * lerp(1.0..=5.0,earthquake.potency);
			}
			for mut velocity in &mut velocities_ii {
				velocity.linvel += Vec3::new(rng.gen_range(-1.0..1.0),rng.gen_range(-1.0..1.0),0.0) * lerp(1.0..=5.0,earthquake.potency);
			}
		}
	}
...
impl ActiveEffects {
	pub fn has_effect(&self, effect_type: EffectType) -> Option<Effect> {
		self.0.iter().filter(|x| x.effect == effect_type).max_by(|a,b| a.potency.total_cmp(&b.potency)).copied()
	}
}
...
pub fn generate_qp_from_ingredients(ingridients: &[Ingredient]) -> (f32,f32) {
	...
	quality += ingridients.iter().filter(|c| c.grind == Grind::Grinded).count() as f32 * 0.10;
...
pub fn generate_effect(effect_quality: &EffectQuality, rng: &mut impl Rng) -> EffectType {
	...
	let choice = match effect_quality {
	    EffectQuality::Catastrophic => {
			const CATASTROPHIC: Choices<EffectType> = choice!(
				Earthquake,
			);
			CATASTROPHIC
		},
	    EffectQuality::Negative => {
			const NEGATIVE: Choices<EffectType> = choice!(
				Slowness,
				Hallucinations,
			);
			NEGATIVE

...
use crate::{
	assets::{SceneInstanceReady, SpawnableArchetype},
	choice,
	prelude::*,
};
...
pub fn set_dither_texture(game_assets: Res<GameAssets>, mut image_assets: ResMut<Assets<Image>>) {
	...
	image_mut.sampler_descriptor = ImageSampler::Descriptor(SamplerDescriptor {
	...
	DITHER_HANDLE
		.set(game_assets.dither_texture.clone())
		.unwrap();
}
		...
		let named_materials = match archetype {
			Tree | Bush => {
				...
				let berry_color = if is_rare {
					...
					Color::hsl(
						rng.gen_range(190.0..360.0),
						rng.gen_range(0.5..0.65),
...
pub fn replace_materials(
	...
	name_query: Query<
		(Entity, &Name, Option<&Handle<FoliageMaterial>>),
		(
			Or<(
				With<Handle<StandardMaterial>>,
				With<Handle<FoliageMaterial>>,
			)>,
			Without<Shadow>,
		),
	>,
	children_query: Query<&Children>,
	spawnable_query: Query<
		(Entity, &NamedMaterials),
		(
			With<SceneInstanceReady>,
			Or<(Added<SceneInstanceReady>, Changed<NamedMaterials>)>,
		),
	>,
) {
	// Don't look at this
	...
	for (parent, material) in &spawnable_query {
		for (child, name, maybe_material) in children_query
			.iter_descendants(parent)
			.filter_map(|child| name_query.get(child).ok())
		{
			let Some(material) = material.0.iter().find(|x| name.contains(&*x.name)).map(|x| x.material) else {
				continue;
			};

			if let Some(Some(child_material)) =
				maybe_material.map(|handle| material_assets.get_mut(handle))
			{
